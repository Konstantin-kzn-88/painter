# -----------------------------------------------------------
# Класс предназначен для отрисовки графической части сервером
#
# Класс реализует:
# - рисование по зонам R1-R6 для заданных коодинат
# - рисование heatmap на основе тангенсального распределения
#   по зонам R1-R6 для заданных коодинат (такое распределние
#   наиболее подходит по пробит функции)
#
...
class Data_draw:
    ...
    def __del_white_pixel(self, pixmap):
        """
        Вспомогательная функция: удаление белых пикселей с картинки
        """
        for i in range(1, 6):
            for k in range(1, 6):
                for j in range(1, 6):
    ...
    def draw_zone(self, object_with_radius: list, size_pic: list, color_zone: list, scale_plan: float):
        """
        Функция рисования зон по координатам и радиусам.
        return QPixmap (для последующего наложения на ген.план
        в клиентской программе)
        """
        print("object_with_radius, size_pic", object_with_radius, size_pic)
        # print("color_zone, scale_plan", color_zone, scale_plan)
        # 2.2. Получить координаты и типы объектов
        print(len(object_with_radius))
        type_obj = []
        coordinate_obj = []
        radius = []
        for obj in object_with_radius:  # получим типы объектов и координаты
            type_obj.append(int(obj[7]))
            coordinate_obj.append(eval(obj[8]))
            radius.append(list(float(i) for i in obj[1:6]))
        print(type_obj)
        # приведем координаты к float
        coordinate_obj = [[float(k) for k in item] for item in coordinate_obj]
        print(coordinate_obj)
        print(radius)
        # 1. создадим соразмерный pixmap_zone и сделаем его прозрачным
        # pixmap_zone = QtGui.QPixmap(size_pic[0], size_pic[1])
